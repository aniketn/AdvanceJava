...
/*Problem Description
        ...
        You have to make a program to display the id of the player who possessed
        the ball after exactly A passes.
...
public class PassingGame {
    public static int solve(int A, int B, int[] C) {
        Stack<Integer> stk = new Stack<>(); // Creating stack
        if (A == 1) {
            return B;
        }
        stk.push(B);
        for (int i = 0; i < A; i++) {
            if (C[i] == 0) { // if value is zero then remove the value from stack(pass ball to previous player)
                stk.pop();
            } else {
                stk.push(C[i]);//if value is not zero then pass the ball to next (push value to stack)
            }
        }
        return stk.peek(); // return current ball holding player
    }
