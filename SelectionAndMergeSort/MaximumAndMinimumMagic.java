...
/*Problem Description
        ...
        Return an array B of size 2, where B[0] = maximum possible value of Magic number modulo 109 + 7, B[1] =
        minimum possible value of a Magic number modulo 109 + 7.
        ...
        Output Format
        Return an array B of size 2, where B[0] = maximum possible value of Magic number % 109 + 7,B[1] =
        minimum possible value of a Magic number % 109 + 7.
...
public class MaximumAndMinimumMagic {
    /*Intuition is like, we can get max difference only when we're subtracting smallest element from largest, and to
    get minimum difference we need to subtract closest/nearest elements. Thus lets sort the array and follow this intuition.*/
    ...
    public static int[] solve(int[] A) {
        ...
        long max = 0, min = 0;
        int mod = 1000000007;
        for (int i = 0; i < A.length / 2; i++) {      //max difference means = largest - smallest
        ...
        for (j = A.length - 1; j >= 0; j -= 2) {
            min += Math.abs(A[j] - A[j - 1]);     //min differnce means = difference of adjacent elements.
            min %= mod;
        }
