...
/*Problem Description
        ...
        laid out in a 2D grid. Our valiant knight was initially positioned
        ...
        Given a 2D array of integers A of size M x N. Find and return
        ...
        for (int i = n - 1; i >= 0; i--) {
            for (int j = m - 1; j >= 0; j--) {
                if (i == n - 1 && j == m - 1) { //Bottom-right cell (Princess Cell)
                    dp[i][j] = Math.min(0, A[i][j]);    //if it is positive celle then put 0
                } else if (i == n - 1) {        //last row
                    dp[i][j] = Math.min(0, dp[i][j + 1] + A[i][j]); //current value + right cell
                } else if (j == m - 1) {        //last column
                    dp[i][j] = Math.min(0, dp[i + 1][j] + A[i][j]); //currnt cell+ down cell
                } else {                        //if we have two option (right and down) then select min of current cell + Max (right or down value cell)
                    dp[i][j] = Math.min(0, Math.max(dp[i][j + 1] + A[i][j], dp[i + 1][j] + A[i][j]));
                }
            }
        }
