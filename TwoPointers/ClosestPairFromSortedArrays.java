...
/*Problem Description
        Given two sorted arrays of distinct integers, A and B, and an integer C, find and return the pair whose
...
public class ClosestPairFromSortedArrays {
    public static int[] solve(int[] A, int[] B, int target) {
        ...
        int closestAIndex = 0, closestBIndex = 0;

        while (start < aSize && end >= 0) {
            int sum = A[start] + B[end];
            int difference = Math.abs(target - sum);

            if (difference < closestDiff || (difference == closestDiff && start <= closestAIndex && end <= closestBIndex)) {
        ...
        return new int[]{
                A[closestAIndex], B[closestBIndex]
        };
    }
